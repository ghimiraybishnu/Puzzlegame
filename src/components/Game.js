...
useEffect(() => {
    ...
    const sourceElement = document.getElementById(tile);
    if (!sourceElement) {
        console.error(`Element with ID ${tile} not found.`);
        return;
    }

    const oldPlace = Number(sourceElement.parentElement.id.slice(6)) - 1;
  ...
  return (
    <div className="h-screen flex text-gray-300 bg-gray-950">
        <div className="mx-auto mt-8">
            ...
            <h3 className="text-x1 font-bold text-center bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90%">
                15 Puzzle Game!
            </h3>
...
<div className="px-6 mt-4">
    <button onClick={newGame} className="text-black font-bold block bg-gray-900 p-2 rounded w-full h-full bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to -90%">
        New Game!
    </button>
</div>
